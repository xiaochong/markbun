...
describe("file lifecycle", () => {
  ...
  it("saves markdown with special characters", async () => {
    await withTrace("file-special-chars", async () => {
      const editor = new EditorPage(page!);
      const filesDir = join(WORKSPACE_DIR, "files");
      await Bun.write(join(filesDir, ".keep"), "");
      await editor.waitForReady();
  ...
  it("saves markdown with nested blockquotes", async () => {
    await withTrace("file-nested-blockquotes", async () => {
      const editor = new EditorPage(page!);
      const filesDir = join(WORKSPACE_DIR, "files");
      await Bun.write(join(filesDir, ".keep"), "");
      await editor.waitForReady();
  ...
  it("saves markdown with hard line breaks", async () => {
    await withTrace("file-hard-breaks", async () => {
      const editor = new EditorPage(page!);
      const filesDir = join(WORKSPACE_DIR, "files");
      await Bun.write(join(filesDir, ".keep"), "");
      await editor.waitForReady();
  ...
  it("saves markdown with task list", async () => {
    await withTrace("file-task-list", async () => {
      const editor = new EditorPage(page!);
      const filesDir = join(WORKSPACE_DIR, "files");
      await Bun.write(join(filesDir, ".keep"), "");
      await editor.waitForReady();
  ...
  it("saves markdown with emoji content", async () => {
    await withTrace("file-emoji-save", async () => {
      const editor = new EditorPage(page!);
      const filesDir = join(WORKSPACE_DIR, "files");
      await Bun.write(join(filesDir, ".keep"), "");
      await editor.waitForReady();
  ...
  it("saves markdown with link content", async () => {
    await withTrace("file-link-save", async () => {
      const editor = new EditorPage(page!);
      const filesDir = join(WORKSPACE_DIR, "files");
      await Bun.write(join(filesDir, ".keep"), "");
      await editor.waitForReady();

      const content = "# Link\n\n[MarkBun](https://example.com)";
      await editor.setMarkdown(content);
      const saveResult = await editor.saveFile(TEST_FILE);
      expect(saveResult.success).toBe(true);
      expect((await Bun.file(TEST_FILE).text()).trim()).toBe(content);
  ...
  it("saves markdown with math block", async () => {
    await withTrace("file-math-block-save", async () => {
      const editor = new EditorPage(page!);
      const filesDir = join(WORKSPACE_DIR, "files");
      await Bun.write(join(filesDir, ".keep"), "");
      await editor.waitForReady();
  ...
  it("saves markdown with image reference", async () => {
    await withTrace("file-image-ref-save", async () => {
      const editor = new EditorPage(page!);
      const filesDir = join(WORKSPACE_DIR, "files");
      await Bun.write(join(filesDir, ".keep"), "");
      await editor.waitForReady();
  ...
  it("saves markdown with strikethrough content", async () => {
    await withTrace("file-strikethrough-save", async () => {
      const editor = new EditorPage(page!);
      const filesDir = join(WORKSPACE_DIR, "files");
      await Bun.write(join(filesDir, ".keep"), "");
      await editor.waitForReady();
  ...
  it("saves markdown with inline code content", async () => {
    await withTrace("file-inline-code-save", async () => {
      const editor = new EditorPage(page!);
      const filesDir = join(WORKSPACE_DIR, "files");
      await Bun.write(join(filesDir, ".keep"), "");
      await editor.waitForReady();
  ...
  it("saves markdown with unordered list content", async () => {
    await withTrace("file-unordered-list-save", async () => {
      const editor = new EditorPage(page!);
      const filesDir = join(WORKSPACE_DIR, "files");
      await Bun.write(join(filesDir, ".keep"), "");
      await editor.waitForReady();

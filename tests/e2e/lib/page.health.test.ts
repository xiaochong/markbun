...
describe("Page health", () => {
  ...
  it("evaluate returns string concatenation", async () => {
    ...
    const runnerExists = await Bun.file(runnerPath).exists();
    if (!runnerExists) {
      console.log("Skipping evaluate string concat test: runner.ts not available.");
      return;
    }
    ...
    try {
      p = await Page.connect();
      expect(await p.evaluate<string>("'hello' + ' ' + 'world'")).toBe("hello world");
      expect(await p.evaluate<string>("'foo'.concat('bar')")).toBe("foobar");
    } catch (err: any) {
      console.log(`Skipping evaluate string concat test: ${err.message}`);
    } finally {
      await p?.close();
    }
  });
  ...
  it("evaluate returns regex test results", async () => {
    ...
    const runnerExists = await Bun.file(runnerPath).exists();
    if (!runnerExists) {
      console.log("Skipping evaluate regex test: runner.ts not available.");
      return;
    }
    ...
    try {
      p = await Page.connect();
      expect(await p.evaluate<boolean>("/markbun/i.test('MarkBun')")).toBe(true);
      expect(await p.evaluate<boolean>("/^\\d+$/.test('abc')")).toBe(false);
    } catch (err: any) {
      console.log(`Skipping evaluate regex test: ${err.message}`);
    } finally {
      await p?.close();
    }
  });
